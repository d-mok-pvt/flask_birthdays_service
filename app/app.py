...
def insert_birthdays_db(birthday_data):
    app.logger.debug(f"Inserting new birthday into database with data: {birthday_data}")
    db_connection = sqlite3.connect(DATABASE)
    uuid_str = str(uuid.uuid4())
    cursor = db_connection.cursor()
    cursor.execute("INSERT INTO birthdays (uuid, name, date) VALUES(?,?,?)", (uuid_str, birthday_data[0], birthday_data[1]))
    db_connection.commit()
    db_connection.close()
    app.logger.info(f"Added new birthday with name {birthday_data[0]} and date {birthday_data[1]} to database")
...
def get_or_post_birthdays():
    if request.method == "GET":
        ...
        if content_type == 'application/x-www-form-urlencoded':
            ...
            data = jsonData["name"], jsonData["date"]
            insert_birthdays_db(data)
            return jsonify({'success':True}), 200

    
@app.route("/api/birthdays/<uuid>", methods=["PUT", "DELETE"])
